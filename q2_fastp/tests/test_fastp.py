...
from q2_fastp.fastp import _find_empty_samples, _remove_samples, _run_fastp, process_seqs
...
class TestFastp(TestPluginBase):
    ...
    def test_find_empty_samples_all(self):
        ...
        for sample in ids:
            os.truncate(os.path.join(self.reads_dir, f"{sample}_00_L001_R1_001.fastq.gz"), 0)
    ...
    def test_remove_samples(self):
        ...
        self.assertFalse(
            os.path.exists(os.path.join(self.reads_dir, "sample1_00_L001_R1_001.fastq.gz"))
        )

        # check that sample2 files still exist
        self.assertTrue(
            os.path.exists(os.path.join(self.reads_dir, "sample2_00_L001_R1_001.fastq.gz"))
        )
    ...
    def test_run_fastp(self, mock_run_command):
        ...
        calls = [
            call([
                "fastp",
                "--in1",
                os.path.join(self.reads_paired.path, f"{s}_00_L001_R1_001.fastq.gz"),
                "--out1",
                ...
                "1",
                "--in2",
                os.path.join(self.reads_paired.path, f"{s}_00_L001_R2_001.fastq.gz"),
                "--out2",
    ...
    def test_process_seqs(self, mock_remove, mock_find_empty, mock_run):
        ...
        mock_find_empty.assert_called_once_with(mock_output_seqs, ["sample1", "sample2"])
